...
export default function Game() {
	...
	useEffect(() => {
		if (matched.size >= settings.gridSize ** 2 || matchedIcons.size >= settings.gridSize ** 2) {
	...
	}, [matched, matchedIcons, refGameSection, refReview, settings.gridSize]);
	...
	useEffect(() => {
		if (settings.theme === "numbers") {
			...
			setSelected([]);
			setMatched(new Set([])); // Adding dummy values to prevent immediate match
		} else {
	...
	function handleClick(i: number) {
		...
		if (selectedIcon.length === 1) {
			...
			const isMatch =
				iconsOfGrid[parseInt(firstIndex)] === iconsOfGrid[parseInt(secondIndex)];
			setTimeout(() => {
				if (isMatch) {
					setMatchedIcons(
